function updateDevisSummary() {
  ...
  const progressBar = document.getElementById("devis-progress");
  const progressLabel = document.getElementById("devis-step");
  const stepOneComplete = name !== "-" && phone !== "-" && city !== "à préciser" && service !== "-";
  const stepTwoComplete = description !== "-";
  if (progressBar) {
    progressBar.classList.toggle("complete", stepOneComplete && stepTwoComplete);
  }
  if (progressLabel) {
    progressLabel.textContent = stepOneComplete ? "Étape 2/2" : "Étape 1/2";
  }
}
